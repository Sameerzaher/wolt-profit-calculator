...
export default function MonthlyPageContent() {
  ...
  const [weeklyGoal, setWeeklyGoal] = useState(readWeeklyGoalFromStorage());
  ...
  return (
    <main className="space-y-4">
      ...
      <section className="grid grid-cols-2 gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-4">
        ...
        <Metric label="ממוצע ₪ לק״מ" value={monthSummary.avgPerKm ? money(monthSummary.avgPerKm) : "-"} />
        <Metric label="המשמרת הכי חזקה" value={monthSummary.bestShift ?? "-"} />
        <Metric label="המשמרת הכי חלשה" value={monthSummary.worstShift ?? "-"} />
      </section>
